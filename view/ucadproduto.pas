...
begin
  ...
  FProdutoController.PreencherGridProdutos(Trim(EdtPesquisar.Text) + '%', LCampo);
...
begin
  ...
  case FOperacao of
    ...
    begin
      try
        FProdutoController.Inserir(FProduto);
        MessageDlg('Produto incluído com sucesso!', mtInformation, [mbOk], 0);
      except
        on E: Exception do
          MessageDlg('Erro ao incluir produto: ' + E.Message, mtError, [mbOk], 0);
      end;
    end;
    ...
    begin
      try
        FProdutoController.Alterar(FProduto, StrToInt(EdtCodProduto.Text));
        MessageDlg('Produto alterado com sucesso!', mtInformation, [mbOk], 0);
      except
        on E: Exception do
          MessageDlg('Erro ao alterar produto: ' + E.Message, mtError, [mbOk], 0);
      end;
    end;
  end;
...
begin
  ...
  begin
    try
      FProdutoController.Excluir(DsProdutos.DataSet.FieldByName('COD_PRODUTO').AsInteger);
